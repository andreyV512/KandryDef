	...
	AnsiString* _reply_data)
{
	...
	if(_reply0.Length()!=0)
		ex->AddReply(_reply0)->data_size=_reply0_size;
	if(_reply1.Length()!=0)
		ex->AddReply(_reply1)->data_size=_reply1_size;
	if(_reply2.Length()!=0)
		ex->AddReply(_reply2)->data_size=_reply2_size;
	bool ret=ex->Exec(comport,_reply,_reply_data);
	error_msg="";
	if(!ret)
